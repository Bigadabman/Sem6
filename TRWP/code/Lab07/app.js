...
async function connectWithRetry() {
  for (let attempt = 1; attempt <= 40; attempt += 1) {
    try {
      ...
      console.log(`TDWA07-01 connected to MSSQL database ${dbName}`);
      return pool;
    } catch (err) {
...
app.get('/health', asyncHandler(async (req, res) => {
  ...
  res.json({ status: 'ok', database: dbName, service: 'TDWA07-01' });
}));
...
app.listen(port, () => {
  console.log(`TDWA07-01 API started on port ${port}`);
});
